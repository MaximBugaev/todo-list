...
export function TodoItem({ title, description, important = false }) {
    ...
    return (
        <li className={isImportant ? "todo-list-item todo-list-item_important" : 'todo-list-item'}>
            <button 
            ...
            <div className="todo-info">
                <div className="title-container">
                    <h2 className="todo-info__title" style={isCompleted ? {textDecoration: 'line-through'} : null}>{ title }</h2>
                    <button 
                        onClick={onClickImportantHandler}
                        className={isImportant ? "todo-info__important-btn todo-info__important-btn_active" : "todo-info__important-btn"}
                    >
                        important
                    </button>
                </div>
                <p className="todo-info__description" style={isCompleted ? {textDecoration: 'line-through'} : null}>{ description }</p>
            </div>
        </li>
    )
}